...
function getTeamsWithAddedAndLostPlayers(){
    return teams.map(team => {
        const teamAdditions = additions.filter(addition => {
            return addition.team_id === team.id
        })
        const teamAddedPlayers = teamAdditions.map(teamAddition => {
            const playerId = teamAddition.player_id
            return players.find(player => {
                return player.id === playerId
            })
        })
        const teamLosses = losses.filter(loss => {
            return loss.team_id === team.id
        })
        const teamLostPlayers = teamLosses.map(teamLoss => {
            const playerId = teamLoss.player_id
            return players.find(player => {
                return player.id === playerId
            })
        })
        team.addedPlayers = teamAddedPlayers
        team.lostPlayers = teamLostPlayers
...
app.get('/', (request, response) => {
    response.json({
        teamsWithAddedAndLostPlayers: getTeamsWithAddedAndLostPlayers()
    })
})
